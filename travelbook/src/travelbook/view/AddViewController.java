...
import travelbook.model.bean.CityBean;
import javafx.scene.input.MouseEvent;
import travelbook.model.bean.StepBean;
import travelbook.util.DateUtil;
import travelbook.util.PlacePrediction;
...
import travelbook.view.animation.OpacityAnimation;
	    ...
	    private void removeStepHandler() {
	    	...
	    	if(result.getButtonData()==ButtonData.OK_DONE) {
	    		...
	    		if(stepsBar.getButtons().size()>0) {
	    			stepsBar.getButtons().remove(stepNumber);
	    			//if stepsBar has some buttons then fire on the first else remove all the information in the field.
	    		}
	    	}
	    }
}
